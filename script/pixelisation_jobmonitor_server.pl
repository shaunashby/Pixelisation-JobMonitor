...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{PIXELISATION_JOBMONITOR_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{PIXELISATION_JOBMONITOR_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
my $runner = sub {
    ...
    require Pixelisation::JobMonitor;

    Pixelisation::JobMonitor->run(
        $port, $host,
        {
            argv       => \@argv,
            'fork'     => $fork,
...
pixelisation_jobmonitor_server.pl - Catalyst Testserver

=head1 SYNOPSIS

pixelisation_jobmonitor_server.pl [options]
